...
export default function RegionPage({ params }: { params: { region: string } }) {
  ...
  const regionDestinations = destinations.filter(d => d.region === params.region)
    ...
    <div style={{ background: '#fafaf9', minHeight: '100vh' }}>
      ...
      <div style={{
        ...
        <div style={{
          position:   'absolute',
          inset:      0,
          background: 'linear-gradient(to top, #0f172a 0%, rgba(15,23,42,0.5) 50%, rgba(15,23,42,0.3) 100%)',
        }} />
        ...
        <div style={{
          ...
          {regionOrder.map(slug => {
            ...
            const isActive = slug === params.region
            return (
              <Link
                key={slug}
                href={`/regions/${slug}`}
                style={{
                  ...
                  borderBottom:   isActive ? `3px solid ${r.accentColor}` : 'none',
                  whiteSpace:     'nowrap',
      ...
      <div style={{ background: '#fff', padding: '48px 0' }}>
        <div style={{
          ...
          {[
            { icon: '🏔', title: 'Great Smoky Mountains', desc: 'Over 800 miles of trails and stunning vistas' },
            { icon: '🌲', title: 'Pristine Wilderness',   desc: '56 state parks and countless natural areas' },
            { icon: '🎵', title: 'Appalachian Culture',   desc: 'Living heritage of music and craftsmanship' },
            { icon: '🏘', title: 'Charming Towns',        desc: 'From Gatlinburg to historic Jonesborough' },
          ].map((f, i) => (
            <div key={i} style={{ textAlign: 'center' }}>
              ...
              <h3 style={{ fontSize: '17px', fontWeight: 700, color: '#1e293b', marginBottom: '6px' }}>{f.title}</h3>
              <p style={{ fontSize: '13px', color: '#475569', lineHeight: 1.55 }}>{f.desc}</p>
            </div>
          ))}
        </div>
      </div>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: '32px' }}>
            <div>
              ...
              <h2 style={{ fontSize: '34px', fontWeight: 400, color: '#1e293b' }}>
                Cities in <strong>{region.label}</strong>
              </h2>
            </div>
        ...
        <div style={{ maxWidth: '1440px', margin: '0 auto', padding: '0 48px' }}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '64px', alignItems: 'start' }}>
            ...
            <div>
                ...
                {region.attractions.map((a, i) => (
                  <Link
                    ...
                    style={{
                      ...
                      padding:      '16px 20px',
                      borderRadius: '12px',
                      background:   hoveredAttraction === a.name ? 'rgba(255,255,255,0.1)' : 'rgba(255,255,255,0.05)',
              ...
              <Link
                href={`/explore?region=${params.region}`}
                style={{
                  display:        'inline-flex',
                  alignItems:     'center',
                  gap:            '8px',
      ...
      <div style={{ background: '#fff', padding: '64px 0' }}>
        <div style={{ textAlign: 'center' }}>
          ...
          <p style={{ fontSize: '14.8px', color: '#475569', marginBottom: '24px' }}>
            Start planning your perfect Tennessee adventure today.
          </p>
          <div style={{ display: 'flex', gap: '12px', justifyContent: 'center' }}>
            <Link href={`/explore?region=${params.region}`} style={{
              display: 'inline-flex', alignItems: 'center', height: '36px', padding: '0 20px',
              borderRadius: '9999px', background: '#f59e0b', color: '#fff',
              fontSize: '13px', fontWeight: 500, boxShadow: '0 1px 3px rgba(0,0,0,.1)',
            }}>
              View All Attractions
            </Link>
            <Link href="/cities" style={{
              display: 'inline-flex', alignItems: 'center', height: '36px', padding: '0 20px',
              borderRadius: '9999px', background: '#fff', color: '#0a0a0a',
              border: '1px solid #e5e5e5', fontSize: '13px', boxShadow: '0 1px 2px rgba(0,0,0,.05)',
            }}>
              Explore Cities
            </Link>
          </div>
