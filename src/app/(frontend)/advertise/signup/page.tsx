...
const TIER_LABELS: Record<
  string,
  { label: string; price: number; color: string; recurring: boolean }
> = {
  basic: { label: 'Basic', price: 29, color: '#475569', recurring: true },
  featured: { label: 'Featured', price: 79, color: '#f59e0b', recurring: true },
  premier: { label: 'Premier', price: 149, color: '#7c3aed', recurring: true },
  article: { label: 'Sponsored Article', price: 399, color: '#0f172a', recurring: false },
}
...
function SignupForm() {
  ...
  const { label, price, color, recurring } = TIER_LABELS[tier]
  const isArticle = tier === 'article'
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (!isArticle) {
        ...
        if (!sponsorRes.ok)
          throw new Error(sponsorData?.errors?.[0]?.message || 'Could not create listing')
      ...
      const checkoutRes = await fetch('/api/stripe/checkout', {
        ...
        body: JSON.stringify({
          ...
          businessUrl: form.website,
          articleTopic: form.articleTopic,
        }),
      })
      const { url, error: checkoutErr } = await checkoutRes.json()
    ...
    <div style={{ minHeight: '100vh', background: '#fafaf9' }}>
      ...
      <div style={{ maxWidth: '600px', margin: '0 auto', padding: '140px 24px 80px' }}>
        <div style={{ textAlign: 'center', marginBottom: '40px' }}>
          ...
          >
            {isArticle ? 'Sponsored Article' : 'Get Featured'}
          </p>
          ...
          <p style={{ fontSize: '15px', color: '#475569', marginBottom: '24px' }}>
            {isArticle ? (
              <>
                One-time payment — <strong style={{ color }}>Sponsored Article — $399</strong>
              </>
            ) : (
              <>
                You selected the{' '}
        ...
        >
          <form onSubmit={handleSubmit}>
            ...
            {isArticle && (
              <div
                style={{ marginBottom: '32px', paddingTop: '32px', borderTop: '1px solid #e5e5e5' }}
              >
                <p
                  style={{
                    fontSize: '12px',
                ...
                <p style={{ fontSize: '13px', color: '#94a3b8', marginBottom: '20px' }}>
                  Our editorial team will write the article — this helps us get started.
                </p>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                  <Field label="What should the article be about?" required>
                    <input
                      type="text"
                      required
                      value={form.articleTopic}
                      onChange={set('articleTopic')}
                      placeholder="e.g. A distillery tour experience, a historic inn, a scenic trail"
                      style={inputStyle}
                    />
                  </Field>
                  <Field label="City or region in Tennessee" required>
                    <input
                      type="text"
                      required
                      value={form.targetLocation}
                      onChange={set('targetLocation')}
                      placeholder="e.g. Lynchburg, Middle Tennessee, Nashville area"
                      style={inputStyle}
                    />
                  </Field>
                  <Field label="Tell us about your business">
                    <textarea
                      ...
                      rows={4}
                      placeholder="What makes your business special? Any key details, history, or talking points you'd like included?"
                      style={textareaStyle}
                    />
                  </Field>
                  <Field label="Anything else?">
                    <textarea
                      ...
                      onChange={set('notes')}
                      rows={2}
                      placeholder="Target audience, seasonal timing, specific requests…"
                      style={textareaStyle}
                    />
                  </Field>
                </div>
            ...
            >
              {loading
                ...
                : isArticle
                  ? 'Continue to payment — $399 one-time'
                  : `Continue to payment — $${price}/mo`}
            </button>

            <p
              style={{ fontSize: '12px', color: '#94a3b8', textAlign: 'center', marginTop: '12px' }}
            >
              {isArticle
                ? 'Secure one-time payment via Stripe. Article published within 5 business days.'
        ...
        <p style={{ textAlign: 'center', marginTop: '24px', fontSize: '13px', color: '#94a3b8' }}>
          Have questions?{' '}
          <a href="mailto:hello@amazingtn.com" style={{ color: '#d97706' }}>
            hello@amazingtn.com
          </a>
          {' · '}
          <Link href="/advertise" style={{ color: '#d97706' }}>
            View all plans
          </Link>
        </p>
      </div>
