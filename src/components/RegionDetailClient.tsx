  ...
  return (
    <div style={{ background: '#fafaf9', minHeight: '100vh' }}>
      ...
      <div style={{ position: 'relative', height: '720px', overflow: 'hidden', background: '#0f172a' }}>
        <img src={region.heroImage} alt={region.label}
        ...
        <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, display: 'flex', justifyContent: 'center' }}>
          {regionOrder.map(slug => {
            const isActive = slug === regionSlug
            const labels: Record<string, string> = { east: 'East Tennessee', middle: 'Middle Tennessee', west: 'West Tennessee' }
            const colors: Record<string, string> = { east: '#059669', middle: '#d97706', west: '#7c3aed' }
            return (
              <Link key={slug} href={`/regions/${slug}`} style={{
                ...
                borderBottom: isActive ? `3px solid ${colors[slug]}` : 'none',
                whiteSpace: 'nowrap',
              }}>
                {labels[slug]}
              </Link>
      ...
      <div style={{ background: '#fff', padding: '48px 0' }}>
        <div style={{ maxWidth: '1440px', margin: '0 auto', padding: '0 48px', display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '32px' }}>
          {region.highlights.map((h, i) => (
            <div key={h} style={{ textAlign: 'center' }}>
              <div style={{ width: '64px', height: '64px', borderRadius: '16px', background: region.gradient, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '28px', margin: '0 auto 16px' }}>
                {['🏔', '🌲', '🎵', '🏘'][i] || '✦'}
              </div>
              <h3 style={{ fontSize: '17px', fontWeight: 700, color: '#1e293b', marginBottom: '6px' }}>{h}</h3>
            </div>
          ))}
        </div>
      </div>
        ...
        <div style={{ maxWidth: '1440px', margin: '0 auto', padding: '0 48px' }}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '64px', alignItems: 'start' }}>
            <div>
              ...
              <Link href={`/explore?region=${regionSlug}`} style={{ display: 'inline-flex', alignItems: 'center', gap: '8px', marginTop: '24px', height: '36px', padding: '0 20px', borderRadius: '9999px', background: region.accentColor, color: '#fff', fontSize: '13px', fontWeight: 500 }}>
      ...
      <div style={{ background: '#fff', padding: '64px 0', textAlign: 'center' }}>
        ...
        <div style={{ display: 'flex', gap: '12px', justifyContent: 'center' }}>
          <Link href={`/explore?region=${regionSlug}`} style={{ display: 'inline-flex', alignItems: 'center', height: '36px', padding: '0 20px', borderRadius: '9999px', background: '#f59e0b', color: '#fff', fontSize: '13px', fontWeight: 500 }}>
